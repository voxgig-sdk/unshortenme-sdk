...
boru-driven command-line client **and** interactive REPL for the Unshortenme
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/unshortenme-cli)
make build

# 2. See usage (words, entities, env vars)
./unshortenme-cli --help

# 3. Provide credentials once, via the environment
export UNSHORTENME_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./unshortenme-cli load 1 unshorten            # {id:1} shorthand
./unshortenme-cli load '{id:1}' unshorten       # explicit match map

# 5. Override the API base URL for a single call
UNSHORTENME_BASE=https://api.example.com ./unshortenme-cli load 1 unshorten

# 6. No arguments -> interactive REPL
./unshortenme-cli
unshortenme> load 1 unshorten
unshortenme> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/unshortenme-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/unshortenme-cli load 1 unshorten
   ```
...
```sh
./unshortenme-cli load 1 unshorten          # scalar shorthand for {id:1}
./unshortenme-cli load '{id:1}' unshorten     # explicit match map
```
...
./unshortenme-cli load 1 unshorten
```
...
Run with no arguments to open a REPL (prompt `unshortenme>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `unshorten`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/unshortenme-cli`. |
...
(`./unshortenme-cli <expr>`) and interactively (the REPL), and expressions compose the
