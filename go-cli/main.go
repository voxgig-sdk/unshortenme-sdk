// Command Unshortenme-cli is a CLI + REPL for the Unshortenme SDK,
...
const prompt = "unshortenme"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "unshorten"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      UNSHORTENME_APIKEY (api key), UNSHORTENME_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: UNSHORTENME_APIKEY carries the API key and
	// UNSHORTENME_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("UNSHORTENME_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("UNSHORTENME_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewUnshortenmeSDK(opts)
