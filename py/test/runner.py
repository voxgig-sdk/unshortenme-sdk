# Unshortenme SDK test runner

from __future__ import annotations
import os
import json

from unshortenme_sdk.utility.voxgig_struct import voxgig_struct as vs


class UnshortenmeTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            UnshortenmeTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = UnshortenmeTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = UnshortenmeTestRunner.getenv("UNSHORTENME_TEST_LIVE")
        override = UnshortenmeTestRunner.getenv("UNSHORTENME_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = UnshortenmeTestRunner.getenv(key)
        ...
        explain = UnshortenmeTestRunner.getenv("UNSHORTENME_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["UNSHORTENME_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if UnshortenmeTestRunner._test_control is not None:
            return UnshortenmeTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                UnshortenmeTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            UnshortenmeTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = UnshortenmeTestRunner.load_test_control()
...
def load_env_local():
    UnshortenmeTestRunner.load_env_local()


def env_override(m):
    return UnshortenmeTestRunner.env_override(m)


def entity_data(v):
    return UnshortenmeTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return UnshortenmeTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return UnshortenmeTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return UnshortenmeTestRunner.load_test_control()


def live_client_options():
    return UnshortenmeTestRunner.live_client_options()


def live_delay_ms():
    return UnshortenmeTestRunner.live_delay_ms()
