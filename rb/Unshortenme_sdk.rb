...
require_relative 'Unshortenme_types'
...
class UnshortenmeSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = UnshortenmeHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = UnshortenmeHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, UnshortenmeFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    UnshortenmeUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = UnshortenmeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = UnshortenmeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = UnshortenmeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = UnshortenmeHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = UnshortenmeError.new(
        "graphql_error", "UnshortenmeSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = UnshortenmeSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
