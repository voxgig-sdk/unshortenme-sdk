...
class UnshortenDirectTest < Minitest::Test
  def test_direct_load_unshorten
    setup = unshorten_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-unshorten", setup[:live] ? "live" : "unit")
    if _should_skip
...
def unshorten_direct_setup(mockres)
  ...
  if live
    ...
    client = UnshortenmeSDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,
