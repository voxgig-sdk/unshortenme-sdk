...
module UnshortenmeUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = UnshortenmeResponse.new({})
    ...
    if fetch_err
      ...
      response = UnshortenmeResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = UnshortenmeResponse.new(fetched)
    else
