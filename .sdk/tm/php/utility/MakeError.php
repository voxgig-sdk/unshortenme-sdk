...
class UnshortenmeMakeError
{
    public static function call(?UnshortenmeContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new UnshortenmeContext([], null);
        }
        $op = $ctx->op ?? new UnshortenmeOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new UnshortenmeResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof UnshortenmeError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new UnshortenmeError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof UnshortenmeError) {
