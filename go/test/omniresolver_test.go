...
import (
	...
	omni "github.com/voxgig-sdk/unshortenme-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.UnshortenmeError); is {
				out := map[string]any{
					"name":    "UnshortenmeError",
...
func omniCtx(arg any, client *sdk.UnshortenmeSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.UnshortenmeSDK); is {
			client = live
