# Unshortenme Golang SDK



The Golang SDK for the Unshortenme API — an entity-oriented client using standard Go conventions. No generics required; data flows as `map[string]any`.
...
func main() {
    ...
    // Load a single unshorten — the value is the loaded record.
...
```go
unshorten, err := client.Unshorten(nil).Load(map[string]any{"url": "example"}, nil)
if err != nil {
    // handle err
    return
}
...
fmt.Println(unshorten) // the returned mock data
```
...
```
UNSHORTENME_TEST_LIVE=TRUE
UNSHORTENME_APIKEY=<your-key>
```
...
```go
func NewUnshortenmeSDK(options map[string]any) *UnshortenmeSDK
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *UnshortenmeSDK
```

Creates a test-mode client with mock transport. Both arguments may be `nil`.

### UnshortenmeSDK methods
...
| `Unshorten` | `(data map[string]any) UnshortenmeEntity` | Create an Unshorten entity instance. |

### Entity interface (UnshortenmeEntity)

All entities implement the `UnshortenmeEntity` interface.
...
slice):
    ...
    if err != nil { /* handle */ }
    // unshorten is the returned record
...
Create an instance: `unshorten := client.Unshorten(nil)`
...
```go
unshorten, err := client.Unshorten(nil).Load(map[string]any{"url": "url"}, nil)
if err != nil {
    panic(err)
}
fmt.Println(unshorten) // the loaded record
```
...
```
github.com/voxgig-sdk/unshortenme-sdk/go/
├── unshortenme.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/unshortenme-sdk/go`) re-exports everything needed
...
unshorten.Load(map[string]any{"url": "example"}, nil)
