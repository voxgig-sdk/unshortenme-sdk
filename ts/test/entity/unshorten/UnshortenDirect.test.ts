...
describe('UnshortenDirect', async () => {
  ...
  test('direct-load-unshorten', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-unshorten', setup.live)) return
