...
//     (\`new UnshortenmeSDK(...)\` / \`UnshortenmeSDK.test(...)\` become
//     \`UnshortenmeSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"unshorten":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new UnshortenmeSDK(...)\` and any
// \`UnshortenmeSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+UnshortenmeSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/UnshortenmeSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
