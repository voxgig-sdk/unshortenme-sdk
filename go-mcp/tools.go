...
type Args struct {
	Entity string         `json:"entity" jsonschema:"unshorten"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.UnshortenmeSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "unshortenme_list",
		Description: "List records from Unshortenme. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "unshortenme_load",
		Description: "Load a single record from Unshortenme. " +
...
func runOp(client *sdk.UnshortenmeSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.UnshortenmeSDK, name string) (sdk.UnshortenmeEntity, error) {
	switch strings.ToLower(name) {
	case "unshorten":
		return client.Unshorten(nil), nil
