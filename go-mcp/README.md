...
[MCP](https://modelcontextprotocol.io) server exposing the Unshortenme SDK as
two agent tools — `unshortenme_list` and `unshortenme_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/unshortenme-mcp)
make build

# 2. Provide credentials via the environment
export UNSHORTENME_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user unshortenme \
  -- /absolute/path/to/unshortenme-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./unshortenme-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// unshortenme_list: first page of records
{ "entity": "unshorten" }
{ "entity": "unshorten", "query": { } }

// unshortenme_load: one record by id
{ "entity": "unshorten", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/unshortenme-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user unshortenme \
     -- "$PWD"/dist/*/unshortenme-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `unshortenme_list` and `unshortenme_load` tools now appear
   in new sessions. Ask the agent to *"list unshorten using unshortenme"*
   and it calls `unshortenme_list` with `{"entity":"unshorten"}`.
...
```sh
export UNSHORTENME_APIKEY=sk_live_xxx            # API key
export UNSHORTENME_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `unshortenme_list` tool
...
### Call the `unshortenme_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `unshortenme_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `unshortenme_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `UNSHORTENME_APIKEY` | API key sent with every request. |
| `UNSHORTENME_BASE` | Optional override of the API base URL. |
...
```sh
./unshortenme-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"unshortenme_load","arguments":{"entity":"unshorten","query":{"id":1}}}}'
```
