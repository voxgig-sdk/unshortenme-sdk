...
class FtCtrl extends UnshortenmeControl
{
...
class FtClient
{
    ...
    private UnshortenmeUtility $utility;

    public function __construct(UnshortenmeUtility $utility, array $options)
    {
    ...
    public function get_utility(): UnshortenmeUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (UnshortenmeContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public UnshortenmeContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (UnshortenmeContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = UnshortenmeConfig::make_feature($name);
    ...
    private static function buildUrl(UnshortenmeSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof UnshortenmeError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new UnshortenmeResponse($fetched);
        }
    ...
    private function fail(UnshortenmeContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (UnshortenmeContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
