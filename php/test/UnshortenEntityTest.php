...
class UnshortenEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = UnshortenmeSDK::test(null, null);
        $ent = $testsdk->Unshorten(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = unshorten_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "unshorten." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set UNSHORTENME_TEST_UNSHORTEN_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $unshorten_ref01_data = null;
        if (count($unshorten_ref01_data_raw) > 0) {
...
function unshorten_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/unshorten/UnshortenTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["unshorten01", "unshorten02", "unshorten03"] as $k) {
    ...
    $entid_env_raw = getenv("UNSHORTENME_TEST_UNSHORTEN_ENTID");
    ...
    if ($env["UNSHORTENME_TEST_LIVE"] === "TRUE") {
        ...
        $client = new UnshortenmeSDK(Helpers::to_map($merged_opts));
    }
