...
The Lua SDK for the Unshortenme API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Unshorten()` — each with the same small set of operations (`load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
```lua
local unshorten, err = client:Unshorten():load({ url = "example" })
if err then error(err) end
```
...
Create an instance: `local unshorten = client:Unshorten(nil)`
...
```lua
local unshorten, err = client:Unshorten():load({ url = "url" })
```
...
```
lua/
├── unshortenme_sdk.lua    -- Main SDK module
...
The main module (`unshortenme_sdk`) exports the SDK constructor
